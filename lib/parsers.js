 ...
 *
 * Means bad progress
 * http://static.managerzone.com/nocache-346/img/training/bar_neg_<num>.png
 * Means bad progress ball lost
 * http://static.managerzone.com/nocache-346/img/training/bar_neg_<num>_ball.png
 *
...
function parsePlayerContainer(html, playerID) {
  let $ = cheerio.load(html);
  let $IDspan = $('span#player_id_' + playerID);
  return $IDspan.parent().parent().parent().html();
}
...
function parseTeamID(clubhouseHTML) {
  //console.log(clubhouseHTML);
  let $ = cheerio.load(clubhouseHTML);
  //console.log($('dl.infoAboutTeam').length);
  let $teamIDContainer = $('.infoAboutTeam').first();
  let ID = new RegExp('<span>\((.+)\)</span>', 'g').exec($teamIDContainer.html())[1];
