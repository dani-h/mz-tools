...
function getCookies(postdata) {
  return Bluebird.coroutine(function * () {
    const res = yield request({method: 'POST',path: '/?p=login'}, postdata);

    assert(res.res.headers.location === '/?p=clubhouse');

    return res.res.headers['set-cookie'];
  })();
}
